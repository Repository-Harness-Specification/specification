...
Before modifying the repository, confirm that these files are attached to the current agent session:
...
They must all come from the same draft `0.2` version, release, or commit.
...
If a required reference is missing, unreadable, or from an inconsistent version, stop and report that the migration cannot be verified.
